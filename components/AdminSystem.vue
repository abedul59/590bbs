...
const editorPassword = ref('590doc')
...
const loadSettings = async () => {
  ...
  if (data) {
    ...
    editorPassword.value = data.editor_password || '590doc'
...
const updateSettings = async () => {
  ...
  await supabase.from('site_settings').upsert({
    ...
    editor_password: editorPassword.value
  })
  currentTheme.value = siteTheme.value
  alert('設定已更新！')
}
...
const pingUrlsByPlatform = async (platform) => {
  ...
  isPinging.value = false; isPingingPlatform.value = ''
